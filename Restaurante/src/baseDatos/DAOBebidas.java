...
/*
    public Libro consultarLibro(Integer idLibro){
        ...
        if (rsLibro.next())
        {
            ...
            {
                 resultado.addCategoria(new Categoria(rsCategorias.getString("categoria"), null, rsCategorias.getInt("numero")));
            }
            } catch (SQLException e){
                 System.out.println(e.getMessage());
                 this.getFachadaAplicacion().muestraExcepcion(e.getMessage());
            ...
            {
                Ejemplar ejemplar = new Ejemplar(resultado, rsEjemplares.getInt("num_ejemplar"),
            ...
            stmPrestamos.setInt(1,resultado.getIdLibro());
            stmPrestamos.setInt(2, ejemplar.getNumEjemplar());
            rsPrestamos=stmPrestamos.executeQuery();
            while (rsPrestamos.next())
            {
                if (rsPrestamos.getDate("fecha_devolucion")==null){
               ...
               stmPrestamos.close();
                ...
                resultado.addEjemplar(ejemplar);
            }
            } catch (SQLException e){
                 System.out.println(e.getMessage());
                 this.getFachadaAplicacion().muestraExcepcion(e.getMessage());
  ...
  public void modificarEjemplarLibro(Integer idLibro, Ejemplar ejemplar){
    ...
    void insertarCategoria(Categoria c) {
        ...
        try {
        stmEjemplar=con.prepareStatement("insert into categoria (nombre, descripcion) "+
                                           "values (?,?)");
        stmEjemplar.setString(1, c.getNombre());
        stmEjemplar.setString(2, c.getDescripcion());
               stmEjemplar.executeUpdate();
    ...
    public void insertarPrestamo(Prestamo p){
        ...
        try {
        stmPrestamo=con.prepareStatement("insert into prestamo (usuario, libro, ejemplar, fecha_prestamo, fecha_devolucion, fecha_vencimiento) "+
                                           "values (?,?, ?, ?, ?, ?)");
        stmPrestamo.setString(1, p.getIdUsuario());
        stmPrestamo.setInt(2, Integer.parseInt(p.getIdLibro()));
        stmPrestamo.setInt(3, Integer.parseInt(p.getIdEjemplar()));
        stmPrestamo.setDate(4,new Date(p.getFechaPrestamo().getTimeInMillis()));
        stmPrestamo.setDate(5, null);
         stmPrestamo.setDate(6,new Date(p.getFechaVencimiento().getTimeInMillis()));
    ...
    void devolverPrestamo(Integer idLibro, Integer idEjemplar) {
        Connection con;
        PreparedStatement stmEjemplar=null;
...
Calendar fecha = Calendar.getInstance();
        ...
        stmEjemplar=con.prepareStatement("update prestamo "+
                                            ...
                                            "where libro=? and ejemplar=?");
        stmEjemplar.setDate(1, new Date(fecha.getTimeInMillis()));
        
        stmEjemplar.setInt(2, idLibro);
        stmEjemplar.setInt(3, idEjemplar);
        stmEjemplar.executeUpdate();
